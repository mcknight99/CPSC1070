...
void Cipher::setCipher(int sizeIn, char* keyIn) {
  cipherQueue.clear();
  unmodifiedQueue.clear();
  for(int i = 0; i<sizeIn; i++) {
    unmodifiedQueue.enqueue(*(keyIn+i));
    cipherQueue.enqueue(*(keyIn+i));
  }
...
std::string Cipher::encodeMessage(std::string in) {
  ...
  //have to reset after messing with the cipher queue
  setCipher(cipherSize, cipher);
  return out;
}
...
std::string Cipher::decodeMessage(std::string in) {
  ...
  setCipher(cipherSize, cipher); //have to reset after messing with the cipher queue
  return out;
}
...
std::string Cipher::inputValidation(std::string in) {
  ...
  for(int i = 0; i<static_cast<int>(in.length()); i++) {
    ...
    if(96<letter&&letter<123) { //lowercase letters range from 97-122, if anything else is in here it should not be added to the key
      out+=letter;
    }
  }
  return out;
