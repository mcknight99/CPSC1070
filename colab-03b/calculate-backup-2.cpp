...
void estimateDriver() {
  ...
  double savings1 = estimateEarnings(currentAge1, retirementAge, principalSavings, monthlyPayments, annualReturnPercentage);
  ...
  cout<<"\nFor starting to save at retirement age " <<currentAge2<<":\n\n";
  double savings2 = estimateEarnings(currentAge2, retirementAge, principalSavings, monthlyPayments, annualReturnPercentage);
  ...
  cout<<"\n\n\nFor a difference in waiting "<< abs(currentAge2-currentAge1) <<" years before starting to invest in retirement, you can miss out potentially on $";
  if(currentAge2>currentAge1) {
    cout<<savings2-savings1<<endl;
      } else if (currentAge1>currentAge2) {
    cout<<savings1-savings2<<endl;
  } else {
    cout<<"theoretically zero dollaroonies because they are the same final age because thats the only last potential option on this forsaken if else statement but why would you put in the same age in twice unless you wanted to see this message";
  }
